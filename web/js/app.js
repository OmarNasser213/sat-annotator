...
class SATAnnotator {
  ...
  async initialize() {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          'Canvas Manager initialization failed: ' + error.message
        );
      }
      ...
      try {
        ...
        throw new Error(
          'Annotation Manager initialization failed: ' + error.message
        );
  ...
  setupEventListeners() {
    // Page reload/close protection - warn if there are uploaded images or annotations
    const beforeUnloadHandler = e => {
      ...
      const hasAnnotations =
        ...
        window.annotationManager.annotations.length > 0;
      ...
      const shouldWarn =
        this.hasUnsavedWork || hasImages || hasAnnotations || hasImagesInDOM;

      if (shouldWarn) {
        console.log('Preventing page unload - user has unsaved work');
        const message =
          'You have unsaved work (uploaded images or annotations). Are you sure you want to leave?';
        e.preventDefault();
    ...
    document
      .getElementById('clearAnnotations')
  ...
  restoreOriginalPolygon() {
    ...
    Utils.showToast(
      `Restored original polygon with ${selectedAnnotation.polygon.length} points`,
      'success'
    );
  ...
  async handleFileSelect(files) {
    ...
    if (invalidFiles.length > 0) {
      Utils.showToast(
        `Invalid files: ${invalidFiles.join(', ')}. Please select valid image files (JPG, PNG, TIFF, GeoTIFF) smaller than 100MB.`,
        'error'
      );
    }
    ...
    console.log(
      `${isSingleUpload ? 'Single' : 'Multi'} upload detected: ${validFiles.length} file(s)`
    );
    ...
    try {
      ...
      for (let i = 0; i < validFiles.length; i++) {
        ...
        try {
          console.log(
            `Starting file upload ${i + 1}/${validFiles.length} for:`,
            file.name
          );
          ...
          const imageData = await api.uploadImage(
            file,
            progress => {
            ...
            false
          ); // Don't show individual loading for multiple uploads

          console.log(
            `Upload ${i + 1}/${validFiles.length} completed:`,
            imageData
          );
          ...
          if (isSingleUpload) {
            ...
            try {
              ...
              console.warn(
                'Preprocessing failed, but image is available for manual annotation:',
                preprocessError
              );
      ...
      if (successfulUploads.length > 0) {
        ...
        if (this.images.length > 0) {
          ...
          const firstUploadedImage = this.images.find(img =>
            uploadedImageIds.includes(img.image_id)
          );
          if (firstUploadedImage) {
            console.log(
              'Selecting first uploaded image with ID:',
              firstUploadedImage.image_id
            );
            ...
            if (!isSingleUpload) {
              // Multi-upload complete - preprocessing selected image for instant segmentation
              ...
              try {
                ...
                console.log(
                  'Selected image preprocessed - first click will be instant'
                );
              } catch (preprocessError) {
                console.warn(
                  'Preprocessing failed for selected image:',
                  preprocessError
                );
  ...
  async selectImage(imageId) {
    ...
    try {
      ...
      console.warn(
        `Preprocessing failed for image ${imageId}:`,
        preprocessError
      );
    }
  ...
  getStatistics() {
    const annotationStats = window.annotationManager
      ? window.annotationManager.getStatistics()
      : {};
    ...
    return {
      ...
      session: {
        ...
        current_tool: window.canvasManager
          ? window.canvasManager.currentTool
          : 'select',
      },
    };
  }
  ...
  exportSession() {
    const sessionData = {
      ...
      annotations: window.annotationManager
        ? window.annotationManager.annotations
        : [],
  ...
  clearSession() {
    ...
    const totalItems =
      this.images.length +
      (window.annotationManager
        ? window.annotationManager.annotations.length
        : 0);

    if (
      confirm(
        `Are you sure you want to clear all data? This will remove ${this.images.length} images and ${window.annotationManager ? window.annotationManager.annotations.length : 0} annotations.`
      )
    ) {
      // Clear images
      this.images = [];
  ...
  async removeImage(imageId) {
    ...
    if (
      !confirm(
        `Are you sure you want to delete the image "${image.file_name}"? This will also remove any annotations on this image.`
      )
    ) {
      return;
    }
  ...
  async clearAllImages() {
    ...
    if (
      !confirm(
        `Are you sure you want to remove all ${imageCount} image${imageCount > 1 ? 's' : ''}? This will also remove all annotations.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      Utils.hideLoading();
      Utils.showToast(
        `Cleared ${imageCount} image${imageCount > 1 ? 's' : ''} and all annotations`,
        'success'
      );
    } catch (error) {
      Utils.hideLoading();
