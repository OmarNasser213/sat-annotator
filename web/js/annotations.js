...
class AnnotationManager {
  ...
  setupEventListeners() {
    ...
    // Custom label input
    document
      .getElementById('addCustomLabel')
      .addEventListener('click', () => this.addCustomLabel());
    document
      .getElementById('customLabelInput')
    ...
    // Context menu events
    document
      .getElementById('deleteAnnotation')
      .addEventListener('click', () => this.deleteSelectedAnnotation());
    document
      .getElementById('editAnnotation')
      .addEventListener('click', () => this.editSelectedAnnotation());
    ...
    document
      ...
      .addEventListener('click', () => this.closeEditLabelModal());
    document
      .getElementById('cancelEditLabel')
      .addEventListener('click', () => this.closeEditLabelModal());
    document
      .getElementById('saveEditLabel')
      .addEventListener('click', () => this.saveEditedLabel());
    ...
    document
      .getElementById('customLabelEdit')
      .addEventListener('keydown', e => {
        if (e.key === 'Enter') {
  ...
  async setCurrentImage(imageId) {
    ...
    if (this.currentImageId && this.annotations.length > 0) {
      try {
        ...
        console.error(
          'Failed to save some annotations before switching images:',
          error
        );
      }
    }
  ...
  async loadAnnotations(imageId) {
    try {
      ...
      this.annotations.sort(
        (a, b) => new Date(a.created) - new Date(b.created)
      );

      console.log(
        `Loaded ${this.annotations.length} annotations:`,
        this.annotations
      );
      this.updateUI();
    } catch (error) {
  ...
  updateRestoreButton() {
    ...
    const selectedAnnotation = this.annotations.find(
      ann => ann.id === this.selectedAnnotation
    );
    const hasOriginal =
      selectedAnnotation &&
      selectedAnnotation.originalPolygon &&
      selectedAnnotation.simplified;
                    ...
                    <div class="annotation-actions">                        <button class="action-btn" onclick="window.annotationManager.editPolygon('${annotation.id}')" title="Edit polygon">
                        ...
                        ${
                          annotation.originalPolygon
                            ? `<button class="action-btn" onclick="window.annotationManager.resimplifyPolygon('${annotation.id}')" title="Re-simplify with current settings">
  ...
  updateAnnotationCounts() {
    ...
    const aiGenerated = this.annotations.filter(
      ann => ann.source === 'ai'
    ).length;
    const manual = this.annotations.filter(
      ann => ann.source === 'manual'
    ).length;
  ...
  saveEditedLabel() {
    ...
    if (
      !this.customLabels.includes(newLabel) &&
    ...
    const annotation = this.annotations.find(
      ann => ann.id === this.currentEditingAnnotationId
    );
    if (annotation && annotation.label !== newLabel) {
      this.updateAnnotation(this.currentEditingAnnotationId, {
        label: newLabel,
      });
  ...
  resimplifyPolygon(id) {
    ...
    if (!annotation || !annotation.originalPolygon) {
      Utils.showToast(
        'No original polygon available for re-simplification',
        'warning'
      );
      return;
    }
    ...
    annotation.simplified =
      annotation.originalPolygon.length !== newSimplified.length;
    ...
    Utils.showToast(
      `Re-simplified polygon: ${annotation.originalPolygon.length} → ${newSimplified.length} points`,
      'success'
    );
  }
  ...
  async deleteAnnotation(id) {
    ...
    if (confirm('Are you sure you want to delete this annotation?')) {
      try {
        ...
        Utils.showToast(
          `Failed to delete annotation: ${error.message}`,
          'error'
        );
      }
    }
  ...
  clearAll() {
    ...
    if (
      confirm(
        `Are you sure you want to delete all ${this.annotations.length} annotations?`
      )
    ) {
      this.annotations = [];
      this.selectedAnnotation = null;
  ...
  showExportModal() {
    ...
    const existingFormatOptions = modal.querySelectorAll(
  ...
  updateExportScopeSelection() {
    ...
    const scopeOptions = document.querySelectorAll(
      '#exportModal .export-scope-option'
    );
    scopeOptions.forEach(option => {
      const radio = option.querySelector('input[type="radio"]');
      if (radio) {
  ...
  async exportCurrentImage(format = 'json') {
    ...
    const exportData = {
      ...
      statistics: {
        ...
        ai_generated: this.annotations.filter(ann => ann.source === 'ai')
          .length,
        manual: this.annotations.filter(ann => ann.source === 'manual').length,
        unique_labels: [...new Set(this.annotations.map(ann => ann.label))]
          .length,
      },
    };
    ...
    Utils.showToast(
      `Exported ${this.annotations.length} annotations for current image as ${format.toUpperCase()}`,
      'success'
    );
  }
  ...
  async exportAllImages(format = 'json') {
    if (
      !window.satAnnotator ||
      !window.satAnnotator.images ||
      window.satAnnotator.images.length === 0
    ) {
    ...
    try {
      ...
      const exportData = {
        ...
        statistics: {
          ...
          ai_generated: allAnnotations.filter(ann => ann.source === 'ai')
            .length,
          manual: allAnnotations.filter(ann => ann.source === 'manual').length,
          unique_labels: [...new Set(allAnnotations.map(ann => ann.label))]
            .length,
      ...
      Utils.showToast(
        `Exported ${totalAnnotations} annotations from ${imageData.length} images as ${format.toUpperCase()}`,
        'success'
      );
    } catch (error) {
      Utils.hideLoading();
      ...
      Utils.showToast(
        'Failed to export all annotations: ' + error.message,
        'error'
      );
    }
  }
  ...
  downloadCSV(data, filename) {
    ...
    if (data.export_info.scope === 'all_images') {
      ...
      csvContent =
        'Number,ID,Image ID,Image Filename,Type,Label,Source,Created,Points Count,Polygon Coordinates\n';

      // Add data rows
      data.annotations.forEach(ann => {
        const polygonStr = ann.polygon
      ...
      csvContent =
        'Number,ID,Type,Label,Source,Created,Points Count,Polygon Coordinates\n';
    ...
    const metadata =
      `# Exported from SAT Annotator on ${data.export_info.exported_at}\n` +
  ...
  drawAnnotations(ctx, scale, offsetX, offsetY) {
    ...
    this.annotations.forEach((annotation, index) => {
      ...
      if (
        !window.canvasManager.imageElement ||
        !window.canvasManager.currentImage
      ) {
        return;
      } // Critical fix: ensure annotations are only drawn for the current image
      console.log(
        `Checking image match - currentImageId: ${this.currentImageId}, canvas image: ${window.canvasManager.currentImage?.image_id}`
      );
      if (window.canvasManager.currentImage.image_id !== this.currentImageId) {
        console.warn(
          `Canvas image mismatch: expected ${this.currentImageId}, got ${window.canvasManager.currentImage.image_id}. Synchronizing...`
        );
      ...
      const isVisible =
        bounds.maxX >= -50 &&
        bounds.minX <= canvasBounds.width + 50 &&
        bounds.maxY >= -50 &&
        bounds.minY <= canvasBounds.height + 50;
      ...
      const imageId = window.canvasManager.currentImage
        ? window.canvasManager.currentImage.image_id
        : null;
      ...
      if (canvasPolygon.length > 0) {
        const centerX =
          canvasPolygon.reduce((sum, p) => sum + p.x, 0) / canvasPolygon.length;
        const centerY =
          canvasPolygon.reduce((sum, p) => sum + p.y, 0) / canvasPolygon.length;
        ...
        this.annotations.length > 0
          ? Math.round(
              this.annotations.reduce(
                (sum, ann) => sum + ann.polygon.length,
                0
