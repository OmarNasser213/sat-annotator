...
class TestSessionImagesAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        async def upload_image(file: UploadFile = File(...)):
            ...
            return {"success": True, "image_id": image.image_id, "file_info": file_info}
        ...
        def delete_image(image_id: str):
            ...
            if self.test_session_id not in session_store.sessions:
                return JSONResponse(
                    status_code=404,
                    content={"success": False, "detail": "Session not found"},
                )

            if "images" not in session_store.sessions[self.test_session_id]:
                return JSONResponse(
                    status_code=404,
                    content={"success": False, "detail": "No images in session"},
                )
    ...
    def test_get_images_empty(self):
        ...
        response = self.client.get(
            "/api/images", cookies={SESSION_COOKIE_NAME: self.test_session_id}
        )
