...
class MockResponse:
    ...
    def set_cookie(self, key, value, expires=None, httponly=False, samesite=None, path=None):
        self.cookies[key] = {
            "key": key,
            "value": value,
            "expires": expires,
...
class TestSessionManager(unittest.TestCase):
    ...
    def test_session_manager_clear_session(self):
        ...
        self.assertTrue(response._deleted_cookies[SESSION_COOKIE_NAME]["httponly"])
...
if __name__ == "__main__":
    ...
    sync_methods = [m for m in dir(TestSessionManager) if m.startswith('test_') and not m.startswith('test_get_session_id')]
