...
class TestSAMSegmenter(unittest.TestCase):
    ...
    def test_set_image(self, mock_cvtcolor, mock_imread):
        ...
        self.assertEqual(
            self.segmenter.cache[self.test_image_path]["image_size"], (768, 1024)
        )
    ...
    def test_predict_from_point(self, mock_cvtcolor, mock_imread):
        ...
        masks = np.array(
            [mock_mask, np.zeros_like(mock_mask), np.zeros_like(mock_mask)]
        )
        ...
        self.assertTrue(
            np.array_equal(
                result[300:500, 400:600], np.ones((200, 200), dtype=np.uint8) * 255
            )
        )
        self.assertTrue(
            np.array_equal(result[0:300, 0:400], np.zeros((300, 400), dtype=np.uint8))
        )
