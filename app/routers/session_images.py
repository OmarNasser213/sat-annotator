...
async def delete_image(
    ...
    try:
        ...
        if success:
            return {
                "success": True,
                "message": "Image and associated annotations deleted successfully",
            }
        else:
            raise HTTPException(
                status_code=500, detail="Failed to remove image from session"
            )
...
def get_session_info(session_manager: SessionManager = Depends(get_session_manager)):
...
def clear_session(session_manager: SessionManager = Depends(get_session_manager)):
...
def export_session(session_manager: SessionManager = Depends(get_session_manager)):
    ...
    if not session_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No session data found"
        )
